...
namespace Blazor.Common.Helper
{
    ...
    public class CCBPayUtil
    {
        ...
        {
            return GetSignContent(strToMap(param));
        }
    ...
    internal class RSASign
    {
        ...
        {
            byte[] inArray = hexStrToBytes(pubKey);
            pubKey = Convert.ToBase64String(inArray);
            string text = "-----BEGIN PUBLIC KEY-----\r\n";
            text += pubKey;
            text += "-----END PUBLIC KEY-----\r\n\r\n";
            byte[] sign2 = hexStrToBytes(sign);
            byte[] bytes = Encoding.GetEncoding(DEFAULT_CHARSET).GetBytes(signContent);
            return RSACheckContent(bytes, sign2, text, "MD5");
        }
    ...
    public class DES_ENCRY_DECRY
    {
        ...
        {
            tdesKey = pubkey.Length > 8 ? pubkey.Substring(0, 8) : pubkey;
            byte[] bytes = DESEncrypt(UTF_16BE, param, ISO_8859_1, tdesKey);
        ...
        {
            tdesKey = pubkey.Length > 8 ? pubkey.Substring(0, 8) : pubkey;
            return DESDecrypt(UTF_16BE, param, ISO_8859_1, tdesKey);
        }
    ...
    internal class RSACryptoServiceProviderExtension
    {
        ...
        private static byte[] GetPublicKeyBlobFromRSA(byte[] RSAData)
        {
            byte[] array = null;
            uint num = 0U;
            if (!CryptDecodeObject((CRYPT_ENCODING_FLAGS)65537U, new IntPtr(19), RSAData, (uint)RSAData.Length, CRYPT_DECODE_FLAGS.NONE, array, ref num))
            {
        ...
        internal static byte[] GetRSAFromDER(byte[] DERData)
        {
            ...
            if (!CryptDecodeObject((CRYPT_ENCODING_FLAGS)65537U, new IntPtr(8), DERData, (uint)DERData.Length, CRYPT_DECODE_FLAGS.NONE, array, ref num))
            {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
            array = new byte[num];
        ...
        internal static byte[] GetDERFromPEM(string sPEM)
        {
            ...
            uint num2;
            uint num3;
            if (!CryptStringToBinary(sPEM, (uint)sPEM.Length, CRYPT_STRING_FLAGS.CRYPT_STRING_BASE64HEADER, null, ref num, out num2, out num3))
            {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
            byte[] array = new byte[num];
            if (!CryptStringToBinary(sPEM, (uint)sPEM.Length, CRYPT_STRING_FLAGS.CRYPT_STRING_BASE64HEADER, array, ref num, out num2, out num3))
            {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
            return array;
        ...
        internal class CRYPT_ALGORITHM_IDENTIFIER
        {
            ...
            internal CRYPT_OBJID_BLOB Parameters = default;
        }
        ...
        private class CERT_PUBLIC_KEY_INFO
        {
            ...
            internal CRYPT_ALGORITHM_IDENTIFIER Algorithm = default;

            // Token: 0x04000030 RID: 48
            internal CRYPT_BIT_BLOB PublicKey = default;
        }
    }
}
